 ...
 * @version $Id: SetCharacterEncodingFilter.java 939521 2010-04-30 00:16:33Z
 *          kkolinko $
 */
...
public class SetCharacterEncodingFilter implements Filter {
	...
	/**
	 * Select and set (if specified) the character encoding to be used to interpret
	...
	private void addSameSite(HttpServletResponse response, String sameSite) {
		...
		for (String header : headers) { // there can be multiple Set-Cookie attributes
			getLogger().debug("Before addSameSite value=" + header);
			if (firstHeader) {
				response.setHeader("Set-Cookie", String.format("%s; Secure; %s", header, "SameSite=" + sameSite));
				firstHeader = false;
				continue;
			}
			response.addHeader("Set-Cookie", String.format("%s; Secure; %s", header, "SameSite=" + sameSite));
		}

		getLogger().debug("After addSameSite value=" + response.getHeaders("Set-Cookie"));
