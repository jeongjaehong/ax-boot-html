...
public class SeedUtil {
	...
	private static byte pbUserKey[] = { 0x64, 0x69, 0x67, 0x69, 0x73, 0x65, 0x65, 0x64, 0x65, 0x6e, 0x63, 0x00, 0x23, 0x20, 0x20, 0x00, 0x00, 0x20, 0x20, 0x51, 0x17, 0x65, 0x65, 0x64, 0x73, 0x65, 0x65, 0x64, 0x73, 0x25, 0x65, 0x64, 0x03, 0x12, 0x04, 0x11 };
	...
	private static void SeedRound(int L0[], int L1[], // [in, out] left-side variable at each round
			final int R0[], final int R1[], // [in] right-side variable at each round
			final int K[]) // [in] round keys at each round
	{
		int T0, T1;
		long T00, T11;
		...
		T11 = T1 < 0 ? (long) T1 & 0x7fffffff | (long) 0x800000 : (long) T1;
		T00 += T11;
	...
	private static void SeedEncrypt(final byte pbData[], // [in] data to be encrypted
			final int pdwRoundKey[], // [in] round keys for encryption
	...
	private static void SeedDecrypt(final byte pbData[], // [in] encrypted data
			final int pdwRoundKey[], // [in] round keys for decryption
	...
	private static void EncRoundKeyUpdate0(int K[], int A[], int B[], final int C[], final int D[], final int Z) {
		int T0;
		int T00, T11;
		T0 = A[0];
		A[0] = (A[0] >>> 8) ^ (B[0] << 24);
	...
	private static void EncRoundKeyUpdate1(int K[], final int A[], final int B[], int C[], int D[], final int Z) {
		int T0;
		int T00, T11;
		T0 = C[0];
		C[0] = (C[0] << 8) ^ (D[0] >>> 24);
	...
	private static void SeedRoundKey(int pdwRoundKey[], // [out] round keys for encryption or decryption
			final byte pbUserKey[]) // [in] secret user key
	{
		int A[] = new int[1];
		int B[] = new int[1];
		int C[] = new int[1];
	...
	public static void setUserKey(final String str) throws UnsupportedEncodingException {
		byte[] encKeyBytes = str.getBytes("UTF-8");
		int keyLength = (encKeyBytes.length < 16) ? 16 : encKeyBytes.length;
		pbUserKey = new byte[keyLength];
		for (int i = 0; i < keyLength; i++) {
			if (i < encKeyBytes.length) {
				pbUserKey[i] = encKeyBytes[i];
			} else {
				pbUserKey[i] = (byte) 0x00;
			}
		}
		SeedRoundKey(pdwRoundKey, pbUserKey);
	}
	...
	// 암호화 함수
	public static String encrypt(final String str) throws UnsupportedEncodingException {
	...
	// 복호화 함수
	public static String decrypt(final String str) throws UnsupportedEncodingException {
	...
	/**
	 * *********** 형변환 함수! ( byte 값에 음수 값이 있어서 str.getBytes(), byte.toString()이 제데로
	 * 동작하지 않아서 만들어 준 함수 ) **************
	 */
	...
	private static void padding(final String str) {
