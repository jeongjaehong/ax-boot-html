...
<!--    <link type="text/css" rel="stylesheet" href="<%=request.getContextPath()%>/sample/assets/fonts/NotoSansKR/NotoSansKR.css" onload="this.media = 'all';" />-->
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/sample/assets/plugins/luplino/luplino-toast/dist/luplino-toast.css"/>
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/sample/assets/plugins/luplino/luplino-menu/dist/luplino-menu.css"/>
    ...
    <script src="<%=request.getContextPath()%>/sample/assets/plugins/luplino/luplino-toast/dist/luplino-toast.js"></script>
    <script src="<%=request.getContextPath()%>/sample/assets/plugins/luplino/luplino-menu/dist/luplino-menu.js"></script>
        ...
        <div class="la-header-cont">
            ...
            <div class="menu-bar">
                <div class="menu-bar-cont">
                    <div class="left">
                        <a href="#" class="logo"><img src="<%=request.getContextPath()%>/sample/assets/img/realgrid-logo.png" style="width:150px; height:50px" /></a>
                        <nav>
                            <div id="GNB-BAR"></div>
                        </nav>
                    </div>
...
let sampleBase = "<%=request.getContextPath()%>/sample";

function normalizeHref(href){
    if(!href) return href;
    if(href.indexOf(sampleBase) === 0) return href;
    if(/realgrid\.com/i.test(href)) return sampleBase + "/about.jsp";
    if(/^https?:\/\//i.test(href)) return href;
    if(href.charAt(0) === "/"){
        let clean = href.replace(/\/+$/, "");
        if(!clean) return href;
        let name = clean.substring(clean.lastIndexOf("/") + 1);
        return sampleBase + "/" + name + ".jsp";
    }
    return href;
}
function normalizeItem(item){
    if(item && item.data && item.data.href){
        item.data.href = normalizeHref(item.data.href);
    }
    return item;
}
...
$(document.body).ready(function(){
    ...
    aside = [
        {
            ...
            data:{href:'<%=request.getContextPath()%>/sample/panel_project.jsp', id:'A001'}
        },
        {
            name:'즐겨찾기',
            icon:'<i class="icon icon-star"></i>',
            data:{href:'<%=request.getContextPath()%>/sample/panel_favorite.jsp', id:'A002'}
        },{
            name:'퀵메뉴',
            icon:'<i class="icon icon-list"></i>',
            data:{href:'<%=request.getContextPath()%>/sample/panel_quick.jsp', id:'A003'}
        }
    ];
    ...
    bside = [
        {
            ...
            data:{href:'<%=request.getContextPath()%>/sample/panel_realgrid.jsp', id:'B001'}
        },
        {
            name:'TO DO LIST',
            icon:'<i class="icon icon-check"></i>',
            data:{href:'<%=request.getContextPath()%>/sample/panel_todolist.jsp', id:'B002'}
        }
    ];
    dashboard = {
        name: '대시보드',
        icon:'<i class=\"icon icon-check\"></i>',
        data: {href:'<%=request.getContextPath()%>/sample/dashboard.jsp', id:'M000'},
    };
    menu = new luplino.ui.menu({
        ...
        items: [
                ...
                children: [
                    {
                        name: '공통코드 관리',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/system_code.jsp', id:'M001001'},
                        role: ''
                    },
                    {
                        name: '프로그램 관리',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/system_app.jsp', id:'M001002'},
                        role: ''
                    },
                    {
                        name: '메뉴 관리',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/system_menu.jsp', id:'M001003'},
                        role: ''
                    },
                    {
                        name: '사용자 관리',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/system_user.jsp', id:'M001004'},
                        role: ''
                    },
                    {
                        name: '루플린',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/system_luplin.jsp', id:'M001005'},
                        role: ''
                    }
                ]
            },
                ...
                children: [
                    {
                        name: '소개',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/about.jsp', id:'M002001'},
                        role: ''
                    ...
                    },
                    {
                        name: '샘플-기본',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_basic.jsp', id:'M002002'},
                        role: ''
                    ...
                    },
                    {
                        name: '샘플-바코드',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_barcode.jsp', id:'M002003'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    },
                    {
                        name: '샘플-필터',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_filter.jsp', id:'M002004'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    },
                    {
                        name: '샘플-그룹핑',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_grouping.jsp', id:'M002005'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    },
                    {
                        name: '샘플-트리뷰',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_treeview.jsp', id:'M002006'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    },
                    {
                        name: '샘플-HTML랜더러',
                        //icon:'<i class="icon icon-check"></i>',
                        data: {href:'<%=request.getContextPath()%>/sample/sample_html.jsp', id:'M002007'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    },
                    {
                        ...
                        data: {href:'<%=request.getContextPath()%>/sample/about.jsp', id:'M002008'},
                        role: ''
                        //accelerator: "CmdOrCtrl+A"
                    }
                ]
            },
            {
                name: '한국프로스포츠관련',
                //icon:'<i class="icon icon-check"></i>',
                data: {href: '<%=request.getContextPath()%>/sample/highchart.jsp', id:'M007'},
                children: []
            },
            {
                name: '거래처',
                //icon:'<i class="icon icon-check"></i>',
                data: {href:'<%=request.getContextPath()%>/sample/client_list.jsp', id:'M003'},
                children: []
            },
            {
                name: '견적서',
                //icon:'<i class="icon icon-check"></i>',
                data: {href:'<%=request.getContextPath()%>/sample/estimate.jsp', id:'M004'},
                children: []
            },
            {
                name: '세금계산서',
                //icon:'<i class="icon icon-check"></i>',
                data: {href:'<%=request.getContextPath()%>/sample/invoice.jsp', id:'M005'},
                children: []
            },
            {
                name: '온실가스배출',
                //icon:'<i class="icon icon-check"></i>',
                data: {href:'<%=request.getContextPath()%>/sample/greengas.jsp', id:'M006'},
                children: []
            },
        ]
    });
    ...
    viewObj.initView = function(){
        ...
        if(ms){
            $.each(ms, function(index, item){
                viewObj.openView(normalizeItem(item));
            });
            if(ma) viewObj.openView(normalizeItem(ma));
        }else{
            viewObj.firstView();
        }
        viewObj.asideView();
    ...
    viewObj.asideView = function(){
        ...
        if(as){
            viewObj.openAside(normalizeItem(as));
        }
    };
    ...
    viewObj.bsideView = function(){
        ...
        if(bs){
            viewObj.openBside(normalizeItem(bs));
        }
    };
